...
routes.post("/clear", async (req, res) => {
    try {
        let { secret, client } = req.headers;
        if(process.env.SECRET === secret) {
            if (await connections.clearClient(client)) {res.status(204).send()} else {res.status(404).send()}
        } else res.status(401).send()
    } catch (error) {
        eLog.writeLog("remote error", error);
        res.status(500).send()
...
routes.delete("/project", async (req, res) => {
    try {
        let { secret, project } = req.headers;
        if(process.env.SECRET === secret) {
            let bcup = await n_Backup();
    
            if(bcup) {//clear all
                let dlt = await project_clear();
...
routes.get("/project", async (req, res) => {
    try {
        ...
        if(process.env.SECRET === secret) {
            ...
            let cProject = await eLog.get_cProject();
            res.status(200).send(cProject);
